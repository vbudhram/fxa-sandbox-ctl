#!/bin/bash
# 10-agent-guide.sh — Bake agent guide and quick-reference into the golden image
set -euo pipefail

echo "==> Installing VM agent guide and context files..."

# ── Full guide: /etc/vm-agent-guide.md ────────────────────────
# This is the complete operations manual (embedded because the source
# repo is not available at image build time).
