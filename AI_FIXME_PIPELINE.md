# ai-fixme, the autonomous ticket-to-PR pipeline
...
```mermaid
flowchart TD
    S0["Stage 0 · Reporter adds the label ai-fixme"]:::human
    S1["Stage 1 · Queued<br/>$CTL queue, oldest first"]:::idle
    S2{"Stage 2 · Admission and grounding<br/>ticket + comments, grep origin/main<br/>10 tool calls"}:::pass
    SKIP["Record skip fingerprint<br/>comment once, then silent"]:::pass
    CLOSE(["Exit · recommend closing<br/>already fixed on main"]):::exit
    ...
    FB{"Unhandled review comments?"}:::pass
    S10{"Stage 10 · drain<br/>PR still open?"}:::pass
    MERGED(["ai-fixme-merged"]):::exit
    REJ(["ai-fixme-rejected"]):::exit
    BLOCK(["ai-fixme-blocked<br/>a cap was reached"]):::exit

    S0 --> S1
    S1 --> S2
    S2 -->|"underspecified or blocked"| SKIP
    SKIP -.->|"reporter answers, fingerprint changes"| S1
    S2 -->|"code already satisfies it"| CLOSE
    S2 -->|"admit"| SLOT
    SLOT -->|"none free"| S1
    SLOT -->|"claim one"| S3
    S3 --> S4
    S4 --> GOAL
    GOAL -->|"no, fail the launch"| BLOCK
    GOAL -->|"yes"| S5
    S5 --> S6
    S6 -->|"handoff file, valid JSON + real work"| S7
    S6 -->|"VM dead, stage watching"| RELAUNCH1
    RELAUNCH1 -->|"no"| S3
    RELAUNCH1 -->|"yes"| BLOCK
    S7 --> REAP
    REAP --> S8
    S8 -->|"all green"| S9
    S8 -->|"flake, under 2 reruns per SHA"| S8
    S8 -->|"real failure, under 2 attempts"| S3
    S8 -->|"cap reached"| BLOCK
    S9 --> FB
    FB -->|"yes, under 2 rounds"| S3
    FB -->|"none"| S10
    FB -->|"2 rounds spent"| BLOCK
    S10 -->|"still open and green"| S9
    S10 -->|"MERGED"| MERGED
    S10 -->|"CLOSED unmerged"| REJ

    classDef pass fill:#1f4e79,stroke:#0d2b45,color:#fff
    classDef orch fill:#2d6a4f,stroke:#15382a,color:#fff
    classDef vm fill:#7b3f00,stroke:#402100,color:#fff
    classDef human fill:#5a3d7a,stroke:#2f1f40,color:#fff
    classDef idle fill:#4a4a4a,stroke:#222,color:#fff
    classDef exit fill:#8b1a1a,stroke:#4a0e0e,color:#fff
```
...
```mermaid
stateDiagram-v2
    [*] --> ai_fixme : reporter adds the label
    ai_fixme : ai-fixme (queued)
    inflight : ai-fixme-inflight (owns a slot)
    done : ai-fixme-done (review queue)
    merged : ai-fixme-merged (archive)
    rejected : ai-fixme-rejected (archive)
    blocked : ai-fixme-blocked (needs a human)

    ai_fixme --> ai_fixme : admission skip, label untouched
    ai_fixme --> inflight : pass claims a slot and launches
    inflight --> ai_fixme : orphaned label, progress reports nolog
    inflight --> inflight : relaunch after a real CI failure
    inflight --> done : PR open and green
    inflight --> blocked : attempt or relaunch cap reached
    done --> inflight : verified review feedback, max 2 rounds
    done --> blocked : feedback cap reached
    done --> merged : drain sees MERGED
    done --> rejected : drain sees CLOSED unmerged
    blocked --> ai_fixme : a human relabels
    merged --> [*]
    rejected --> [*]
```
